...
subroutine movecrate(crate_reg, crate_reg_len, from, to)
    character(len=1), dimension(9, 8*9) :: crate_reg
    integer, dimension(9) :: crate_reg_len
    integer :: from, to
    character(len=1) :: temp_char

    ! hold the crate to move
    temp_char = crate_reg(from, crate_reg_len(from))
    print *, "moving ", temp_char, " from", from, "to", to
    print * , crate_reg_len

    ! "remove" the crate
    crate_reg(from, crate_reg_len(from)) = " "
    crate_reg_len(from) = crate_reg_len(from) - 1

    ! "add" the crate
    crate_reg(to, crate_reg_len(to)+1) = temp_char
...
program part1
    ...
    do 
        ...
        do ix = 1, move_num
            call movecrate(crate_reg, crate_reg_len, from_ix, to_ix)
        end do
    end do
